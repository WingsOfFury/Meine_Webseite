...
Ich kannte mich mit dem Fetchen zwar aus und Astros herangehensweise gefiel mir auch sehr gut aber habe mich trotzdem für meine Variante entschieden.


## Was ist eine API

`API` steht für `Application Programming Interface` und was es genau macht und wie es funktioniert erfährst du im nächsten Absatz.

Eine schöne Erklärung von [AWS](https://aws.amazon.com/de/what-is/api/ "Amazon Web Services") :<br/>
...
Hier verdeutliche ich dir nochmal wie ich die Daten angezapft habe.
Das Zauberwort heißt "Fetch" und bedeutet nichts anderes als sich die Daten zu "bringen/holen".
Es gibt viele Fetch Methoden um an die Daten heran zu kommen.
Hier ist ein Link für dich
Ich habe mich für die von React entschieden.

Das erste Bild ist ein Beispiel aus der Documentation von Astro:
...
Das zweite Bild ist von meinem Code:
...
Das dritte Bild zeigt dir die JSON Datei des Anbieters an wo alle Daten einer Person aufgelistet ist.
...
Du kannst jeden verfügbaren Parameter anzapfen den du möchtest.
Wenn du z.B. ein Bild haben möchtest dann schaut der Code so aus.
Das ist so klasse an APIs.
...
nur sehen kann ob die Daten auch wirklich alle da sind die man angezapt hat.
Wenn alles geklappt hat und die gewünschten Daten fließen dann kann man auch
die Seite zu Ende stylen.

## Wirst du die Seite weiter bearbeiten 

Ja, ich werde mich später darum kümmern mehrere Personen
nebeneinander gleichzeitig anzeigen zu lassen.
Das Design werde ich auch ändern.
